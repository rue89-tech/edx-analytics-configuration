...
remote-task --job-flow-name="$CLUSTER_NAME" --repo "https://github.com/rue89-tech/edx-analytics-pipeline.git" --branch "rue89-release" --wait --log-path $WORKSPACE/logs/ --remote-name automation --user hadoop --sudo-user hadoop --override-config "$OVERRIDE_CONFIG" "$@"
